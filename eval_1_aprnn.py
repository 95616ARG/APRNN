...
if not args.use_artifact:
    ...
    """ Specify the symbolic weights.
        ==============================
        `N[k:].requires_symbolic_weight_and_bias(lb=-3., ub=3.)` makes the
        k-th layer weight and l-th layer bias symbolic where l >= k.
    """
    N[k:].requires_symbolic_weight_and_bias(lb=-3., ub=3.)

    """ Encode the symbolic output.
        ===========================
        `N(images)` symbolically forwards `images` through `N` with the default
        reference points `images`. It is equivalent to

        ```
        activation_pattern = N.activation_pattern(images)
        symbolic_output = N(images, pattern=activation_pattern)
        ```
    """
    ...
    result_path = (get_results_root() / 'eval_1' / f'aprnn_{args.net}').as_posix()

else:
    N = network.deepcopy()
    N.load((get_artifact_root() / 'eval_1' / f'aprnn_{args.net}.pth'))
    time = None

    result_path = (get_results_root() / 'eval_1' / f'artifact_aprnn_{args.net}').as_posix()
...
result = {
    args.net: {
        ('APRNN', 'D'): d0 - d1,
        ('APRNN', 'G'): g1 - g0,
        ('APRNN', 'T'): 'N/A' if time is None else f'{int(time)}s',
    }
}
...
if args.use_artifact:
    print_msg_box(
        f"Experiment 1 for MNIST {args.net} using APRNN SUCCEED.\n"
