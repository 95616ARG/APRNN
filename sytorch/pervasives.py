...
__all__ = [
    ...
    'interval_mul_interval_bound_of_bound',
    'Interval',
    'multiply_interval_with_variable_overapprox',
    'multiply_interval_with_variable_linearize',
    'multiply_symbolic_interval_with_concrete',
]
...
@_export
def multiply_interval_with_variable_linearize(interval, variables):
    """_summary_

    Args:
        interval (Array[..., I, 2]): _description_
        variables (SymbolicArray[..., I]): _description_

    Returns:
        SymbolicArray[..., I, 2]: _description_
    """

    solver = variables.solver

    # approx lb for lb, ub for ub
    out_approx_bounds = solver.reals((*variables.shape, 2))
    out_exact_bounds = interval * variables[:,None]
    solver.add_constraints(
        out_approx_bounds[...,:1] <= out_exact_bounds,
        out_approx_bounds[...,1:] >= out_exact_bounds,
    )
    return out_approx_bounds

def multiply_interval_with_interval_bound(interval, other_interval):
    """ Worst-cast bound.

    Args:
        interval (Array[..., I, 2]): _description_
        other_interval (Array[..., I, 2]): _description_

    Returns:
        Array[..., I, 2]: _description_
    """
...
@_export
def multiply_symbolic_interval_with_concrete(symbolic_interval, concrete):
    """_summary_

    Args:
        symbolic_interval (SymbolicArray[..., I, 2]): _description_
        concrete (Array[..., I]): _description_

    Returns:
        SymbolicArray[..., I, 2]: _description_
    """

    solver = symbolic_interval.solver
    out_approx_bounds = solver.reals(symbolic_interval.shape)
    out_exact_bounds = symbolic_interval * concrete[:,None]
    solver.add_constraints(
        out_approx_bounds[...,:1] <= out_exact_bounds,
        out_approx_bounds[...,1:] >= out_exact_bounds,
    )
...
@_export
def multiply_interval_with_variable_overapprox(interval, variables):
    vbound = variables.bounds
    xlb ,xub = interval[...,0], interval[...,1]
    wlb, wub = vbound[...,0], vbound[...,1]

    # For now we assume intervals are from ReLU hence non-negative.
    assert (xlb >= 0.).all()

    # if (xlb >= 0.).all() and (wlb <= 0.).all() and (wub >= 0.).all():
    k = (xlb * wub - xub * wlb) / (wub - wlb)
    b0 = (xlb - k) * wub
    b1 = (xub - k) * wub

    mask_semi_positive = wlb >= 0.
    mask_over_approx = (wlb < 0.) * (wub > 0.)
    mask_semi_negative = wub <= 0.

    warnings.warn('assert check')
    assert (mask_semi_positive + mask_over_approx + mask_semi_negative).all()
    out_bound = np.empty(interval.shape, dtype=object)

    # print(mask_semi_positive.sum())
    # print(mask_over_approx.sum())
    # print(mask_semi_negative.sum())

    if mask_semi_positive.any():
        out_bound[mask_semi_positive, 0] = interval[mask_semi_positive, 0] * variables[mask_semi_positive]
        out_bound[mask_semi_positive, 1] = interval[mask_semi_positive, 1] * variables[mask_semi_positive]

    if mask_over_approx.any():
        out_bound[mask_over_approx, 0] = k[mask_over_approx] * variables[mask_over_approx] + b0[mask_over_approx]
        out_bound[mask_over_approx, 1] = k[mask_over_approx] * variables[mask_over_approx] + b1[mask_over_approx]

    if mask_semi_negative.any():
        out_bound[mask_semi_negative, 0] = interval[mask_semi_negative, 1] * variables[mask_semi_negative]
        out_bound[mask_semi_negative, 1] = interval[mask_semi_negative, 0] * variables[mask_semi_negative]
...
# @overload
# def _interval_mul_interval_spacing_bound(int1, int2, V, h): ...
# @_unwrap_closure
# def _interval_mul_interval_spacing_bound():
#     LB, UB, FLIP = as_slice[0], as_slice[1], as_slice[::-1]
#     LL, UU, LU, UL = tuple(as_slice[...,i] for i in range(4))

#     def pos(interval):
#         return interval[LB] >= 0.

#     def neg(interval):
#         return interval[UB] <= 0.

#     def impl(int1, int2, V, h):
#         """_summary_

#         Args:
#             int1 (Array[2]): _description_
#             int2 (Array[2]): _description_
#             V (Array[4]): V = int1 * int2 :: int1 * int2[::-1]
#             h (Array[2]): ref out bound of ub-lb

#         Returns:
#             _type_: _description_
#         """

#         pos_int1, neg_int1, pos_int2, neg_int2 = pos(int1), neg(int1), pos(int2), neg(int2)
...
@overload
def _interval_mul_interval_bound_of_bound(int1, int2, V, h, lb_int, ub_int): ...
@_unwrap_closure
def _interval_mul_interval_bound_of_bound():
    LB, UB, FLIP = as_slice[0], as_slice[1], as_slice[::-1]
    LL, UU, LU, UL = tuple(as_slice[...,i] for i in range(4))
    ...
    def impl(int1, int2, V, h, lb_int, ub_int):
        """_summary_

        Args:
            int1 (Array[2]): _description_
            int2 (Array[2]): _description_
            lb_int (Array[2]): bound of lb
            ub_int (Array[2]): bound of ub
        ...
        if pos(int1):
            ...
            if pos(int2):
                ...
                # h[LB] = 0.
                # h[UB] = max(V[UU] - V[LU], V[LL] - V[UL])
                lb_int[LB] = min(V[UL], V[LU])
                lb_int[UB] = 0  # max(V[UL], V[LU])
                ub_int[LB] = 0  # min(V[LL], V[UU])
                ub_int[UB] = max(V[LL], V[UU])

    return impl

def interval_mul_interval_bound_of_bound(int1, int2):
    ...
    # h_bound = np.zeros_like(int1)
    h_bound = None
    lb_bound = np.zeros_like(int1)
    ub_bound = np.zeros_like(int1)
    # print(np.prod(int1.shape[:-1]))
    for idx in np.ndindex(int1.shape[:-1]):
        _interval_mul_interval_bound_of_bound(
            int1 = int1[idx],
            int2 = int2[idx],
            V = vertices[idx],
            h = None, # h_bound[idx],
    ...
    return lb_bound, ub_bound, h_bound

@_export
class Interval(ndarray):
    def __mul__(self, other):
        self = self.view(ndarray)
        other = other.view(ndarray)

        """ Straight worst case bound. But we can also find the worst case H. """
        comb = np.concatenate((self * other, self[...,::-1] * other), axis=-1)
    ...
    #     if (xlb >= 0.).all() and (wlb <= 0.).all() and (xub >= 0.).all():
    #         k = (xlb * wub - xub * wlb) / (wub - wlb)
    #         b0 = (xlb - k) * wub
    #         b1 = (xub - k) * wub

    #         return np.stack((k * w + b0, k * w + b1), axis=-1)
