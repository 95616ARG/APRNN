...
class Module(nn.Module):
    ...
    def to_eran_bounds(self, boxes):
        assert boxes.ndim == 3 # NCB
        return boxes

    # def h(self, boxes, domain='deeppoly'):
    #     if self.has_symbolic_parameter:
    #         raise NotImplementedError
    #     device, dtype = self.device, self.dtype
    #     net_eran = self.to_eran()
    #     output_boxes = []

    #     for box in boxes:
    #         analyzer, (element, lbs, ubs) = net_eran.get_abstract0(box[:,0], box[:,1], domain=domain)
    #         output_boxes.append(torch.stack((torch.tensor(lbs, dtype=dtype, device=device), torch.tensor(ubs, dtype=dtype, device=device)), -1))
...
class ONNXContainer(Module, ONNXCompatibleModule):
    ...
    def h(self, boxes, domain='deeppoly'):
        ...
        Args:
            boxes (Tensor[N, C, 2] or Tensor[N, C, W, H, 2]): _description_
            domain (str, optional): _description_. Defaults to 'deeppoly'.
        """
        assert boxes.ndim in [3, 5]
        device, dtype = self.device, self.dtype

        sample_input = boxes[...,0]
        sample_output = self(sample_input)
        assert sample_output.ndim in [2, 4]

        """ Transpose input NCHW to NHWC. """
        if sample_input.ndim == 4:
            boxes = torch.moveaxis(boxes, -4, -2).flatten(-4, -2)

        output_boxes = []
        for box in tqdm(boxes, desc='deeppoly boxes'):
            assert box.ndim == 2
            analyzer, (element, lbs, ubs) = self.eran.get_abstract0(
                                                box[:,0], box[:,1], domain=domain)
            output_boxes.append(
                torch.stack((
                    torch.tensor(lbs[-1], dtype=dtype, device=device),
                    torch.tensor(ubs[-1], dtype=dtype, device=device)
                ), -1)
            )

        output_boxes = torch.stack(output_boxes, 0)
        if sample_output.ndim == 4:
            eran_output_shape = tuple(torch.moveaxis(sample_output, -3, -1).shape)
            output_boxes = output_boxes.reshape(*eran_output_shape, 2)
            output_boxes = torch.moveaxis(output_boxes, -2, -4)
        else:
            eran_output_shape = tuple(sample_output.shape)
            output_boxes = output_boxes.reshape(*eran_output_shape, 2)
