...
class AdaptiveAvgPool2d(LinearLayer, Layer2D, nn.AdaptiveAvgPool2d):
    ...
    def h(self, boxes, **kwargs):
        if isinstance(boxes, SymbolicLightningArray):
            return SymbolicLightningArray.stack((
                self.forward_symbolic(boxes[...,0]),
                self.forward_symbolic(boxes[...,1]),
            ), axis=-1)
...
class MaxPool2d(NonLinearLayer, Layer2D, nn.MaxPool2d):
    ...
    def h(self, boxes, **kwargs):
        ...
        if isinstance(boxes, SymbolicLightningArray):
            ...
            # ni(wh)jk
            window_views_lb = self.as_strided(boxes[...,0])
            window_views_ub = self.as_strided(boxes[...,1])

            N, I, WH, J, K = window_views_lb.shape

            output = solver.reals((N, I, J, K, 2))
            output_broadcasted = broadcast_at(output, 2, (WH,))
            handle_constrs(output_broadcasted[..., 0] >= window_views_lb)
            handle_constrs(output_broadcasted[..., 1] >= window_views_ub)
