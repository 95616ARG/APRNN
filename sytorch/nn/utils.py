...
def _import_eran(eran_path=None):
    if pathlib.Path('ERAN').exists():
        eran_path = './ERAN/tf_verify'
        warnings.warn(f"loading ERAN from {pathlib.Path('ERAN')}")

    if eran_path is not None:
        import sys
        if eran_path not in sys.path:
            sys.path.insert(0, eran_path)
            sys.path.insert(0, f"{eran_path}/../ELINA/python_interface/")
            sys.path.insert(0, f"{eran_path}/../deepg/code/")
        # from eran import ERAN
        # return ERAN
        import eran
        if eran_path in sys.path:
            sys.path.remove(eran_path)
            sys.path.remove(f"{eran_path}/../ELINA/python_interface/")
            sys.path.remove(f"{eran_path}/../deepg/code/")
        # with _with_paths(
        #     eran_path,
        #     f"{eran_path}/../ELINA/python_interface/",
        #     f"{eran_path}/../deepg/code/"
        # ):
        #     import sys, importlib.util, pathlib
        #     eran_path = pathlib.Path(eran_path)
        #     # if eran_path.is_dir() or eran_path.suffix != ".py":
        #     #     eran_path /= "__main__.py"
        #     _eran_spec = importlib.util.spec_from_file_location("eran", eran_path)
        #     eran = importlib.util.module_from_spec(_eran_spec)
        #     sys.modules["earn"] = eran
        #     # _eran_spec.loader.exec_module(eran)
    else:
        import eran

    return eran

def to_eran(net, sample_input=None, earn_path=None, stop_at=None):
    import onnx
    # eran = _import_eran(earn_path)
    from sytorch.external.eran import eran
    if isinstance(net, Module):
        net = to_onnx(net, sample_input=sample_input)
    assert isinstance(net, onnx.ModelProto)
    return eran.ERAN(net, is_onnx=True, stop_at=stop_at)

# def _get_eran():
#     eran = _import_eran(None)
#     import onnx
#     return eran.ERAN(onnx.load('data/eran/nets/onnx/mnist/convSmallRELU__Point.onnx'), is_onnx=True)
