...
parser.add_argument('--device', type=str, dest='device', action='store', default='cuda',
                    help='device to use, e.g., cuda, cuda:0, cpu. (default=cuda).')
...
if args.net == 'resnet152':
    ...
    raise RuntimeError(
        f"Unsupported args.net {args.net}"
    )
...
if not args.use_artifact:
    ...
    repair_dataset = imagenet.datasets.ImageNet_A()\
            .to(dtype=dtype, device=device)\
            .misclassified(net, num=50, seed=None)
    ...
    net = net.decouple().to(solver)\
            .repair()\
            .requires_symbolic_(False)

    net[k].val.weight.requires_symbolic_(lb = -10., ub = 10.)
    ...
    result_path = (get_results_root() / 'eval_2' / f'prdnn_{args.net}').as_posix()

else:

    print_msg_box(
        "In our run, Experiment 2 using PRDNN ran out of memory and FAILED."
    )
    quit()
    time = None
    result_path = (get_results_root() / 'eval_2' / f'artifact_prdnn_{args.net}').as_posix()
...
print_msg_box(
    f"Experiment 2 using PRDNN for {args.net} SUCCEED.\n"
